...
import { requireSpaceMembership, normalizeSpacePath } from '@/lib/private-space';
import { CreatePrivateSpaceFileSchema } from '@/lib/validations/schemas';
import { z } from 'zod';
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = CreatePrivateSpaceFileSchema.parse(body);
    const folderPath = normalizeSpacePath(data.folderPath);
    ...
    console.error('[PRIVATE_SPACE_CREATE_FILE]', error);
